...
const handleRegister = async () => {
  ...
  try {
    await authStore.registerEndUser({
...
<template>
  <div class="auth-view">
    <header class="auth-view__hero">
      <RouterLink to="/login" class="auth-view__back">返回登录</RouterLink>
      <h1>用户注册</h1>
      <p>注册成功后会自动登录，并恢复到你刚才准备访问的页面。</p>
    </header>

    <main class="auth-view__content">
      <section class="auth-card">
        <h2>创建 end_user 账号</h2>
        <p>只做最小可用注册流程，账号唯一、密码加密存储。</p>

        <label class="auth-field">
          <span>昵称</span>
          <input v-model="form.displayName" type="text" autocomplete="nickname" placeholder="请输入昵称" />
        </label>
        ...
        <button type="button" class="auth-card__primary" :disabled="isSubmitting" @click="handleRegister">
          {{ isSubmitting ? '注册中...' : '注册并登录' }}
        </button>

        <RouterLink class="auth-card__secondary" :to="{ path: '/login', query: route.query }">
          已有账号，去登录
        </RouterLink>
      </section>
    </main>
