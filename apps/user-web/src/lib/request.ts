...
const buildRequestInit = (options: RequestOptions, token?: string) => {
...
const refreshAccessToken = async (baseURL = DEFAULT_BASE_URL) => {
  ...
  refreshPromise = (async () => {
    ...
    if (!response.ok) {
      clearSessionAndNotify()
      throw new RequestError(resolveErrorMessage(response, payload), response.status, payload)
    }
...
export const setRequestAuthHandlers = (handlers: SessionUpdateHandlers) => {
